...
def run_experiment(
    ...
    trainer = Trainer(model, optimizer, "regression", entropy_coe)
    ...
    for epoch in tqdm(range(num_epochs), "Epoch"):
        ...
        train_rmse_list.append(train_rmse_eval)
...
def main():
    ...
    base_cfg = SuperParamters(
        ...
        num_exp = 4,        # 默认值，下面会覆盖
        out_dim = 1,
        num_epochs = 30,   # 建议先从 200/300 试，2000 太狠了
        init_lr=1e-3,
        warmup_steps=1000, 
        decay_steps=12000,    
        end_lr=1e-5,
    ...
    exps.append(("TopkGate-1E", cfg_soft1, "moe"))

    # 3. SoftGate, 4 experts
    cfg_soft4 = dataclasses.replace(
        base_cfg,
        num_exp=4,
        choose_gate_name="topk",
        entropy_coe=1e-2,
    )
    exps.append(("TopkGate-4E", cfg_soft4, "moe"))
