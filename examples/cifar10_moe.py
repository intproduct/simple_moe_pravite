...
def load_cifar10():
    ...
    mean = mx.array([0.4914, 0.4822, 0.4465], dtype=mx.float32)
    std  = mx.array([0.2023, 0.1994, 0.2010], dtype=mx.float32)

    mean_flat = mx.repeat(mean, repeats=32*32)
    std_flat  = mx.repeat(std, repeats=32*32)

    X_train = mx.array(X_train.reshape(-1, 32*32*3))
    X_test  = mx.array(X_test.reshape(-1, 32*32*3))
...
def build_model(
    ...
    dim_model=512, num_experts=10, dim_hidden=256,
    gate_name="topk", k=1, temperature=1.0, num_exp_layers=1,
    entropy_coeff=1e-2
):
    model = GenericMoE(
        dim_input=input_dim,
